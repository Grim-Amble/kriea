...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
        return this.getVectorForRotation(EventMotion.getPitch(), EventMotion.getYaw());
    }
    ...
    public void onUpdate()
    {
    	...
    	Client.getModuleManager().onEvent(event);
    ...
    private void onUpdateWalkingPlayer()
    {
        ...
        {
        	EventMotion event = new EventMotion(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch, this.lastReportedYaw, this.lastReportedPitch, this.onGround);
            Client.getModuleManager().onEvent(event);
            
            double diffX = event.getPosX() - event.getPrevPosX();
            double diffY = event.getPosY() - event.getPrevPosY();
            double diffZ = event.getPosZ() - event.getPrevPosZ();
            double diffYaw = (double)(event.getYaw() - event.getPrevYaw());
            double diffPitch = (double)(event.getPitch() - event.getPrevPitch());
            ++this.positionUpdateTicks;
            boolean shouldUpdatePos = diffX * diffX + diffY * diffY + diffZ * diffZ > 9.0E-4D || this.positionUpdateTicks >= 20;
            ...
            else if (event.isPrevOnGround() != event.isOnGround())
            {
            ...
            if (shouldUpdatePos)
            {
                event.setPrevPosX(event.getPosX());
                event.setPrevPosY(event.getPosY());
                event.setPrevPosZ(event.getPosZ());
                this.positionUpdateTicks = 0;
            }

            if (shouldUpdateLook)
            {
                event.setPrevYaw(event.getYaw());
                event.setPrevPitch(event.getPitch());
            }

           event.setPrevOnGround(event.isOnGround());
