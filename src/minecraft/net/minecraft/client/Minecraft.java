...
import me.valrod.client.ui.clickgui.ClickGUI;
...
public class Minecraft implements IThreadListener, ISnooperInfo
{
    ...
    private void createDisplay() throws LWJGLException
    {
        ...
        Display.setTitle(Client.CLIENT_NAME + " " + Client.CLIENT_VERSION + " | Base made by Valrod");
    ...
    public static void placeBlock(BlockPos pos)
    {
        if (!Minecraft.getMinecraft().playerController.getIsHittingBlock())
        {
            ...
            if (!Minecraft.getMinecraft().player.isRowingBoat())
            {
                ...
                for (EnumHand enumhand : EnumHand.values())
                {
                    ItemStack itemstack = Minecraft.getMinecraft().player.getHeldItem(enumhand);
                    ...
                    if (Minecraft.getMinecraft().objectMouseOver != null)
                    {
                        ...
                        {
                            case ENTITY:
                                if (Minecraft.getMinecraft().playerController.interactWithEntity(Minecraft.getMinecraft().player, Minecraft.getMinecraft().objectMouseOver.entityHit, Minecraft.getMinecraft().objectMouseOver, enumhand) == EnumActionResult.SUCCESS)
                                {
                                    return;
                                }

                                if (Minecraft.getMinecraft().playerController.interactWithEntity(Minecraft.getMinecraft().player, Minecraft.getMinecraft().objectMouseOver.entityHit, enumhand) == EnumActionResult.SUCCESS)
                                {
                                    return;
                                }
                            ...
                            case BLOCK:
                                ...
                                if (Minecraft.getMinecraft().world.getBlockState(blockpos).getMaterial() != Material.AIR)
                                {
                                    int i = itemstack.func_190916_E();
                                    EnumActionResult enumactionresult = Minecraft.getMinecraft().playerController.processRightClickBlock(Minecraft.getMinecraft().player, Minecraft.getMinecraft().world, blockpos, Minecraft.getMinecraft().objectMouseOver.sideHit, Minecraft.getMinecraft().objectMouseOver.hitVec, enumhand);

                                    if (enumactionresult == EnumActionResult.SUCCESS)
                                    {
                                    	Minecraft.getMinecraft().player.swingArm(enumhand);

                                        if (!itemstack.func_190926_b() && (itemstack.func_190916_E() != i || Minecraft.getMinecraft().playerController.isInCreativeMode()))
                                        {
                                        	Minecraft.getMinecraft().entityRenderer.itemRenderer.resetEquippedProgress(enumhand);
                                        }
                    ...
                    if (!itemstack.func_190926_b() && Minecraft.getMinecraft().playerController.processRightClick(Minecraft.getMinecraft().player, Minecraft.getMinecraft().world, enumhand) == EnumActionResult.SUCCESS)
                    {
                    	Minecraft.getMinecraft().entityRenderer.itemRenderer.resetEquippedProgress(enumhand);
                        return;
                    }
    ...
    private void runTickKeyboard() throws IOException
    {
        while (Keyboard.next())
        {
            ...
            if (flag)
            {
                ...
                if (this.currentScreen == null)
                {
                	Client.getModuleManager().onKeyPressed(i);
                    if (i == 1)
                    {
