...
public abstract class RenderLivingBase<T extends EntityLivingBase> extends Render<T>
{
    ...
    public void doRender2(T entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
    	entityYaw = EventMotion.getYaw();
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, partialTicks, x, y, z))
        {
            ...
            try
            {
                float f = this.interpolateRotation(EventMotion.getPrevYaw(), EventMotion.getPrevYaw(), partialTicks);
                float f1 = this.interpolateRotation(EventMotion.getPrevYaw(), EventMotion.getPrevYaw(), partialTicks);
                float f2 = f1 - f;

                if (this.mainModel.isRiding && entity.getRidingEntity() instanceof EntityLivingBase)
                {
                    EntityLivingBase entitylivingbase = (EntityLivingBase)entity.getRidingEntity();
                    f = this.interpolateRotation(EventMotion.getPrevYaw(), EventMotion.getPrevYaw(), partialTicks);
                    f2 = f1 - f;
                ...
                float f7 = EventMotion.getPrevPitch() + (EventMotion.getPitch() - EventMotion.getPrevPitch()) * partialTicks;
    ...
    protected void rotateCorpse(T entityLiving, float p_77043_2_, float p_77043_3_, float partialTicks)
    {
        ...
        else
        {
            ...
            if (s != null && ("Dinnerbone".equals(s) || "Valrod".equals(s) || "ValrodClient".equals(s) || "Grumm".equals(s)) && (!(entityLiving instanceof EntityPlayer) || ((EntityPlayer)entityLiving).isWearing(EnumPlayerModelParts.CAPE)))
            {
    ...
    public void renderName(T entity, double x, double y, double z)
    {
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, entity, this, x, y, z))
        {
            if (this.canRenderName(entity))
            {
                ...
                if (d0 < (double)(f * f) || Client.getModuleManager().isModuleOn(new Nametags()))
                {
